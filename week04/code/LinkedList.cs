...
public class LinkedList : IEnumerable<int>
{
    ...
    {
        // If the list is empty or has only one item, set head and tail to null.
        if (_tail == _head)
        {
            _head = null;
            _tail = null;
        }
        // If the list has more than one item, adjust the links to remove the tail.
    ...
    public void Remove(int value)
    {
        ...
        while (current is not null)
        {
            if (current.Data == value)
            {
                ...
                {
                    current.Prev!.Next = current.Next; // Connect previous node to the next node
                    current.Next!.Prev = current.Prev; // Connect next node to the previous node
                }
