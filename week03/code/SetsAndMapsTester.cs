...
public static class SetsAndMapsTester
{
    ...
    private static void DisplayPairs(string[] words)
    {
        ...
        {
            // we simple reverse the current word
            string reverseElement = new(new char[] { element[1], element[0] });

            // and check if the reversed word exists in the set
            if (pairs.Contains(reverseElement))
            {
                // Display the pair
                Console.WriteLine($"{reverseElement} & {element}");
            }

            // if not we add the current word to the set and repeat
            pairs.Add(element);
        }
    }
